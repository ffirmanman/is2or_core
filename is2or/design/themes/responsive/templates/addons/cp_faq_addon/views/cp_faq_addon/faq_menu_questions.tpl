...
{elseif $items}
	{foreach from=$items item="quest"}
		{if !$block_items_ids}
			{assign var="block_items_ids" value=$quest.back_items_ids}
		{/if}
	{/foreach}
{elseif $product.faq_menu_questions}
	{foreach from=$product.faq_menu_questions item="quest"}
		{if !$block_items_ids}
			{assign var="block_items_ids" value=$quest.back_items_ids}
		{/if}
	{/foreach}
{/if}
{if $items}
	{assign var="faq_questions" value=$items}
	...
	{/if}
{/if}
{if $product.faq_menu_questions}
	{assign var="faq_questions" value=$product.faq_menu_questions}
{/if}



<div id="cp_faq_questions_page_{$faq_pref}">
	{if $show_block_searh && $show_block_searh == "Y"}
		<div class="ty-search-block">
			<form action="{""|fn_url}" method="post" name="search_faq_quest_form" id="search_faq_quest_form" class="cm-ajax cm-submit">
				<input type="hidden" name="result_ids" value="cp_faq_questions_page_{$faq_pref}">
				{*
				<input type="hidden" name="faq_type" value="question">
				{if $block.type}
					<input type="hidden" id="faq_block_type" name="faq_block_type" value="{$block.type}">
				{/if}
				*}
				{if $no_hide_answers}
				...
				{/if}
				{if $block_items_ids}
					<input type="hidden" id="faq_block_questions_items" name="block_sect_items_ids" value="{$block_items_ids}">
				{/if}
				...
				{/if}
					{strip}
					<div id="magnifere_but">{include file="buttons/magnifier.tpl" but_name="cp_faq_addon.faq_quest_search" alt=__("search") }</div>
	...
	{else}
		...
		{/if}
    {/if}
<!--cp_faq_questions_page_{$faq_pref}--></div>
<script language="javascript">
    $(document).ready(function(){
        ...
        $('#question_'+quest_id+'_'+prefix).removeClass("hidden");
        if (quest_id) {
