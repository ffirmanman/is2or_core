{foreach $product_features as $feature}
    {if $feature.feature_type != "ProductFeatures::GROUP"|enum}
        ...
        <div class="ty-product-feature">
                ...
                <div class="ty-product-feature__value">
            ...
            </span>{if $feature.value === "YesNo::YES"|enum}{hook name="ab__product_features:variant"}{__('ab__sf.checkbox_set')}{/hook}{/if}
                    ...
                    {elseif $feature.feature_type == "ProductFeatures::MULTIPLE_CHECKBOX"|enum && $feature.variants}
                        <ul class="ty-product-feature__multiple">
                            {foreach from=$feature.variants item="var"}
                                ...
                                {if $var.selected}<li class="ty-product-feature__multiple-item"><span class="ty-compare-checkbox">{include_ext file="common/icon.tpl" class="ty-icon-ok ty-compare-checkbox__icon"}</span>{if !$hide_variant_affix}<span class="ty-product-feature__prefix">{$feature.prefix}</span>{/if}{hook name="ab__product_features:variant"}{$var.variant}{/hook}{if !$hide_variant_affix}<span class="ty-product-feature__suffix">{$feature.suffix}</span>{/if}</li>{/if}
                            {/foreach}
                        </ul>
                    {elseif in_array($feature.feature_type, ["ProductFeatures::TEXT_SELECTBOX"|enum, "ProductFeatures::EXTENDED"|enum, "ProductFeatures::NUMBER_SELECTBOX"|enum])}
                        {foreach $feature.variants as $variant}
                            {if $variant.selected}{hook name="ab__product_features:variant"}{$variant.variant}{/hook}{break}{/if}
                        {/foreach}
