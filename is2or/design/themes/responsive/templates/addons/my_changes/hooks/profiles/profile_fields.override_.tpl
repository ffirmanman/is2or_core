...
{else}
<div class="ty-control-group ty-profile-field__item ty-{$field.class} {if $field.field_type == "ProfileFieldTypes::PHONE"|enum || $field.autocomplete_type == "phone-full"}cm-mask-phone-group{/if} {if preg_match('/field-hide/', $field.class) && $runtime.storefront_id == 3}cm-skip-validation hidden{/if}" {if $field.field_type == "ProfileFieldTypes::PHONE"|enum || $field.autocomplete_type == "phone-full"}data-ca-phone-mask-group-id="{$element_id}"{/if}>
