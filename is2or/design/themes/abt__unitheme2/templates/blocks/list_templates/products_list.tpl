                    ...
                    {if $settings.abt__ut2.product_list.{$tmpl}.enable_hover_gallery.{$settings.ab__device} === "YesNo::NO"|enum}
                        <div class="ty-center-block">
                            <div class="ty-thumbs-wrapper owl-carousel cm-image-gallery ty-scroller"
                                ...
                                {else}
                                    <span class="ty-no-image" style="aspect-ratio: {$tbw|intval} / {$tbh|intval};">{include_ext file="common/icon.tpl" class="ty-icon-image ty-no-image__icon" title=__("no_image")}</span>
                                {/if}
                                {$fewer_items = []}
