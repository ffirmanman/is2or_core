                    ...
                    <ul class="ty-product-filters__variants ty-product-filters__color-filter" {if $filter.display_count}style="--ut2-product-filter-shortlist-variant-count: {$filter.display_count};"{/if}>
                        {foreach from=$filter.variants  item=variant name=color_variants}
                            <li class="cm-product-filters-checkbox-container {if $smarty.foreach.color_variants.iteration > $filter.display_count}hidden{else}ty-product-filters__color-list-item{/if}">
                                <label for="elm_checkbox_{$filter_uid}_{$variant.variant_id}" class="ty-product-filters__color-filter-item {if $variant.disabled}{if $variant.selected}ty-product-filters__color-filter-item--selected{else}ty-product-filters__color-filter-item--disabled{/if}{/if}" data-cm-product-color-filter="true" data-ca-product-color-filter-checkbox-id="elm_checkbox_{$filter_uid}_{$variant.variant_id}" title="{$filter.prefix}{$variant.variant|fn_text_placeholders}{$filter.suffix}">
                                    <input
                                    ...
                                    <span class="ty-product-filters__color-filter-swatch {if $variant.selected}ty-product-filters__color-filter-swatch--selected{/if}"
