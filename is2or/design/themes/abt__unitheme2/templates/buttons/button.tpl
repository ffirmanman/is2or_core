...
{if $but_name && $but_role != "text" && $but_role != "act" && $but_role != "delete"} {* SUBMIT BUTTON *}
    <button {if $but_id}id="{$but_id}"{/if} class="{$but_meta} ty-btn" type="submit" name="{$but_name}" {if $but_onclick}onclick="{$but_onclick nofilter}"{/if} aria-label="{if $but_text}{$but_text}{else}Action button{/if}">{if $but_icon}<span><i class="{$but_icon}"></i>{/if}{if $but_text}<bdi>{$but_text}</bdi>{/if}{if $but_icon}</span>{/if}</button>
