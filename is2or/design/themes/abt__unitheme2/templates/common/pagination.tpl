...
{if $pagination.total_pages > 1}
    ...
    <div class="ty-pagination" id="ut2_pagination_block{if $smarty.capture.pagination_open == "Y"}_bottom{/if}">
        ...
        {/if}
        <a data-ca-scroll=".cm-pagination-container" class="ty-pagination__item ty-pagination__btn {if $pagination.prev_page}ty-pagination__prev cm-history {$ajax_class} {$ajax_full_render_class}{/if}" {if $pagination.prev_page}href="{"`$c_url`&page=`$pagination.prev_page`"|fn_url}" data-ca-page="{$pagination.prev_page}" data-ca-target-id="{$id|cat:$extra_id}"{/if}>{include_ext file="common/icon.tpl" class="ut2-icon-arrow_back_black"}&nbsp;<span class="ty-pagination__text">{__("prev_page")}</span></a>
