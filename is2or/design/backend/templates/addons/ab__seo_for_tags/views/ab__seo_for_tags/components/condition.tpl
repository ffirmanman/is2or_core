{$condition_name="ab__seo_for_tags.conditions.names.`$condition_data.condition`"}
{$prefix_md5 = $prefix|md5}
{$condition_schema = $schema[$condition_data.condition]}
{if $condition_schema}
...
{/if}
{hook name="ab__seo_for_tags:condition"}
{if $condition_schema.type == 'chained'}
<div class="select2-wrapper--width-auto">
<select name="{$prefix}[condition_element]" id="ab__sft_chained_condition_parent_{$prefix_md5}">
...
<select title="" name="{$prefix}[operator]" id="ab__sft_condition_operator_{$prefix_md5}">
{foreach $condition_schema.operators as $operator}
<option value="{$operator}"{if $operator == $condition_data.operator} selected{/if}>{__("promotion_op_`$operator`")}</option>
{/foreach}
</select>
{else}
{$operator = reset($condition_schema.operators)}
<span style="width:196px;display:inline-block">{__("promotion_op_`$operator`")}</span>
{/if}
{/if}
...
{include file="common/calendar.tpl" date_name="{$prefix}[value]" date_id="ab__sft_date_{$prefix_md5}" date_val=$condition_data.value|default:$smarty.const.TIME}
...
<option value="{$_k}"
{if $_k == $condition_data.value}selected{/if}>{if $schema[$condition_data.condition].variants_function}{$v}{else}{__($v)}{/if}</option>
{/foreach}
</select>
...
<div class="select2-wrapper--width-auto">
<select name="{$prefix}[value]"{if $condition_data.operator == 'in' || $condition_data.operator == 'nin'} multiple{/if} class="hidden" id="ab__sft_chained_condition_child_{$prefix_md5}">
...
<input id="ab__sft_chained_condition_child_input_{$prefix_md5}" type="text" name="{$prefix}[value]" value="{$condition_data.value}"
class="hidden input-long"/>
<script>
(function (_, $) {
$(document).ready(function () {
var chainedCondition = new _.ChainedPromotionConditionForm({
operatorSelect: $('#ab__sft_condition_operator_{$prefix_md5}'),
parentSelect: $('#ab__sft_chained_condition_parent_{$prefix_md5}'),
childSelect: $('#ab__sft_chained_condition_child_{$prefix_md5}'),
childInput: $('#ab__sft_chained_condition_child_input_{$prefix_md5}'),
settings: {
parent: {
dataUrl: "{$schema[$condition_data.condition].chained_options.parent_url|fn_url}"
}
}
});
