{if $search_history} 
  ...
  <table width="100%" class="table table-sort table-middle table--relative table-responsive">
    ...
    <tr>
        ...
        {/if}
        <th width="5%"></th>  
        <th width="40%"><a class="cm-ajax" href="{"`$c_url`&sort_by=search_word&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__('request')}{if $search.sort_by == "search_word"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
    ...
    <tbody>
        <tr class="cm-longtap-target">
            ...
            {/if}
            <td>{if $i.is_turbo}<div class="cls-request-turbo-label" title="{__('cls.from_turbo')}"></div>{/if}</td>
            <td data-th="{__("request")}">
            	<span title="{$i.q}">{$i.q|truncate:50}</span>              
