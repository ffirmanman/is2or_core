...
{capture name="buttons"}
    {capture name="tools_list"}
        ...
        {if $runtime.action=="per_request" || $runtime.action=="per_word"}
        	<li>{btn type="list" class="" text=__("export") href= fn_url("exim.export?section=csc_live_search&pattern_id=cls_history_`$runtime.action`")}</li>
        {/if}
...
{capture name="sidebar"}
   ...
   {if !$runtime.company_id || fn_allowed_for('ULTIMATE')}
    <h6>{__('settings')}</h6>	
    ...
    {include file="addons/`$addon_base_name`/views/`$addon_base_name`/components/history_search_form.tpl" period=$search.period search=$search dispatch="`$runtime.controller`.`$runtime.mode`.`$runtime.action`"} 
