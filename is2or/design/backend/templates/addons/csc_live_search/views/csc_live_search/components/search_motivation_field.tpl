...
{if $field.alt_field_name}
	{$field_name = $field.alt_field_name}	
{/if}
...
<div class="control-group cls-search-motivation" id="container_elm_{$field_name}">
  ...
  <div class="controls">  
  	<div class="cls-synonyms input-large" data-ca-keys="enter" data-ca-name="{$param_name}[{$input_name}]" >
        {foreach from=$options.$input_name item=phrase}			
           <div class="tag">{$phrase}</div>
        {/foreach}
        <input type="hidden" value="array(){json_encode($options.$input_name)}" class="tags_array" name="{$param_name}[{$input_name}]" />
    </div>
