  ...
  <table width="100%" class="table table-sort table-middle table--relative table-responsive">
    ...
    <tbody>
        <tr class="cm-longtap-target">
            ...
            <td data-th="{__("product")}"><a href="{fn_url("profiles.update?user_id=`$i.user_id`")}">{$i.firstname} {$i.lastname} ({$i.email})</a>
            	{include file="views/companies/components/company_name.tpl" object=$i}
            </td> 
            <td data-th="{__("cls.click")}" class="center"><a title="{__('cls.click')}: {$i.product}" class="cm-ajax cm-dialog-opener" href="{"csc_live_search.history.per_product?cls_uid=`$i.user_id`&in_popup=Y"|fn_url}" data-ca-target-id="search_history_per_request" data-ca-view-id="{$i.user_id}">{$i.clsClicks}</a></td>
            <td data-th="{__("cls.phrases")}" class="center">
                {if $i.clsPhrases > 0}
                    <a title="{__('cls.phrases')}: {$i.product}" class="cm-ajax cm-dialog-opener" href="{"csc_live_search.history.per_word?user_id=`$i.user_id`&in_popup=Y"|fn_url}" data-ca-target-id="search_history_per_word" data-ca-view-id="{$i.user_id}">{$i.clsPhrases}</a>
                {else}
                    0
                {/if}
            </td>
