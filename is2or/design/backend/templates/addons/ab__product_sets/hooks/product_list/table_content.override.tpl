{if $smarty.request.data_id|strpos:'ab__ps_set' !== false}
{hook name="product_list:table_content"}
{assign var="time" value=$smarty.now}
{if $hide_amount}
<td class="center" width="1%"><input type="{if $show_radio}radio{else}checkbox{/if}" name="{$checkbox_name}[]" value="{$product.product_id}" class="cm-item mrg-check" id="checkbox_id_{$time}_{$product.product_id}" /></td>
{/if}
<td>
{hook name="product_list:product_data"}
<input type="hidden" id="product_{$product.product_id}" value="{$product.product}" />
{if $hide_amount}
<label for="checkbox_id_{$time}_{$product.product_id}">{$product.product nofilter}</label>
{else}
