...
{/if}
{hook name="tabs:ab__update_nav_tabs"}{/hook}
</ul>
</div>
...
<div class="controls">
<input type="hidden" name="tab_data[show_in_popup]" value="{"YesNo::NO"|enum}" />
<input type="checkbox" name="tab_data[show_in_popup]" id="elm_show_in_popup_{$html_id}" {if $tab_data.show_in_popup == "YesNo::YES"|enum}checked="checked"{/if} value="YesNo::YES"|enum>
</div>
</div>
{if $tab_data.is_primary !== "YesNo::YES"|enum && "block_manager.update_block"|fn_check_view_permissions}
<div class="control-group">
...
<div class="controls">
<label class="radio text-value">{if $tab_data.status == "ObjectStatuses::ACTIVE"|enum}{__("active")}{else}{__("disabled")}{/if}</label>
</div>
</div>
<input type="hidden" class="cm-no-hide-input" name="snapping_data[object_type]" value="{$dynamic_object_scheme.object_type}" />
<div class="control-group">
<label class="control-label">{if $tab_data.status == "ObjectStatuses::ACTIVE"|enum}{__("disable_for")}{else}{__("enable_for")}{/if}:</label>
<div class="controls">
{include_ext
...
{/if}
{hook name="tabs:ab__tabs_content"}{/hook}
</div>
