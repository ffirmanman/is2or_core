...
{script src="js/addons/csc_virtual_categories/backend.js"}
...
{capture name="set"}
{if $group.set == "any"}
{assign var="selected_name" value=__("cvc.cond_any")}
{else}
{assign var="selected_name" value=__("cvc.cond_all")}
{/if}
{include file="common/select_object.tpl" style="field" items=["all" => __("cvc.cond_all"), "any" => __("cvc.cond_any")] select_container_name="`$prefix`[set]" selected_key=$group.set selected_name=$selected_name}
{/capture}

{capture name="set_value"}
{if !$group || $group.set_value}
{assign var="selected_name" value=__("cvc.cond_true")}
{else}
{assign var="selected_name" value=__("cvc.cond_false")}
{/if}
{include file="common/select_object.tpl" style="field" items=["0" => __("cvc.cond_false"), "1" => __("cvc.cond_true")] select_container_name="`$prefix`[set_value]" selected_key=$group.set_value|default:1 selected_name=$selected_name}
{/capture}
...
<ul class="conditions-tree-group cm-row-item">
    ...
    {foreach from=$group.conditions key="k" item="condition_data" name="conditions"}
		...
		<li id="container_condition_{$prefix_md5}_{$k}" class="cm-row-item{if $smarty.foreach.conditions.last} cm-last-item{/if}">
			{if !empty($condition_data) && !empty($condition_data.set)} {* this is the group *}
				{include file="addons/csc_virtual_categories/components/condition_group.tpl" root=false group=$condition_data prefix="`$prefix`[conditions][`$k`]" elm_id="condition_`$prefix_md5`_`$k`"}
			{else}
				{include file="addons/csc_virtual_categories/components/conditions.tpl" condition_data=$condition_data prefix="`$prefix`[conditions][`$k`]" elm_id="condition_`$prefix_md5`_`$k`" zone="catalog"}
			{/if}
		</li>  
		{/if}
    {/foreach}
    ...
    <li id="container_add_condition_{$prefix_md5}" class="hidden cm-row-item">
        ...
        <select onchange="Tygh.$.ceAjax('request', '{"categories.cvc_dynamic?category_id=`$smarty.request.category_id`"|fn_url nofilter}&prefix=' + encodeURIComponent(this.name) + '&condition=' + this.value + '&elm_id=' + this.id, {$ldelim}result_ids: 'container_' + this.id{$rdelim})">
            <option value=""> -- </option>
            {foreach from=$schema.conditions key="_k" item="c"}
                {assign var="l" value="cvc.cond_`$_k`"}
