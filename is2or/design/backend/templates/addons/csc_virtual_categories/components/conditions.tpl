{assign var="l" value="cvc.cond_`$condition_data.condition`"}

<div class="conditions-tree-node clearfix cvc_{$condition_data.condition}_condition">
    ...
    {if $schema.conditions[$condition_data.condition].type == "mixed"}
        ...
        <select name="{$prefix}[condition_element]" id="mixed_condition_element_{$p_md}" style="width:200px;">
    ...
    {if $schema.conditions[$condition_data.condition].type == "chained"}
        <div class="select2-wrapper--width-auto">
            <select name="{$prefix}[condition_element]" id="promotion_chained_condition_parent_{$p_md}" style="width:200px;">
    ...
    {if $schema.conditions[$condition_data.condition].type != "list" && $schema.conditions[$condition_data.condition].type != "statement"}
        <select name="{$prefix}[operator]" id="promotion_condition_operator_{$p_md}" style="width:200px;">
    ...
    {if $schema.conditions[$condition_data.condition].type == "input"}
     ...
     {elseif $schema.conditions[$condition_data.condition].type == "date"}       	
        
        {include file="common/calendar.tpl" date_id=rand(1500, 10000) date_name="`$prefix`[value]" date_val=$condition_data.value start_year=$settings.Company.company_start_year}
        
        
    {elseif $schema.conditions[$condition_data.condition].type == "select"}
        <select name="{$prefix}[value]" style="width:200px;">
    ...
    {elseif $schema.conditions[$condition_data.condition].type == "picker"}    	
        ...
        {$name_suffix = ''}
        {if $params.input_name_suffix}
        	{$name_suffix = $params.input_name_suffix}
        {/if}
        
        {include_ext 
        	file=$schema.conditions[$condition_data.condition].picker_props.picker 
        	company_ids=$picker_selected_companies data_id="objects_`$elm_id`" 
            input_name="`$prefix`[value]`$name_suffix`" 
            item_ids=$condition_data.value 
            params_array=$params 
            owner_company_id=$promotion_data.company_id 
            but_meta="btn"
        }
    ...
    {elseif $schema.conditions[$condition_data.condition].type == "chained"}
        <div class="select2-wrapper--width-auto" >
            <select name="{$prefix}[value]" {if $condition_data.operator == 'in' || $condition_data.operator == 'nin'}multiple{/if} class="hidden" id="promotion_chained_condition_child_{$p_md}" style="width:200px;">
    ...
    {hook name="csc_virtual_categories:conditions"}{/hook}
