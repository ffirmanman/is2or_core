(function(_, $) {
  ...
  function _is2orFillCustomerAddress(data, isShipping) {
    ...
    $('#' + latFieldId).val(data.Latitude).attr('data-ca-latest-latitude', data.Latitude).trigger('change');
    $('#' + lonFieldId).val(data.Longitude).attr('data-ca-latest-longitude', data.Longitude).trigger('change');
