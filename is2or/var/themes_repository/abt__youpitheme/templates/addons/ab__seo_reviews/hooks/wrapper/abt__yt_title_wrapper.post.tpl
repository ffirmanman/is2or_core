{if $reviews_page.posts_count}
    ...
    <a href="{fn_url("ab__seo_reviews.view_category?category_id=`$reviews_page.object_id`")}" title="" class="ab-sr__link"><i class="ty-icon-chat"></i> {$reviews_page.posts_count} {__("reviews", [$reviews_page.posts_count])}</a>
    </span>
{/if}