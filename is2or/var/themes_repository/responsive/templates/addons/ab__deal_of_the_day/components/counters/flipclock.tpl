<script>
    (function (_, $) {
        $.ceEvent('on', 'ce.commoninit', function (context) {
            const $block = context.find('#ab__deal_of_the_day_{$block.block_id}');
            if (!$block.length) {
                return;
            }
            $.getScript('js/addons/ab__deal_of_the_day/lib/flipclock.min.js?v={$addons.ab__deal_of_the_day.build}', function () {
                var total_seconds = Number({$total_seconds});

                _.ab__dotd.clock_{$block.block_id} = $block.FlipClock(total_seconds, {
                    countdown: true,
