...
class ClsSearchPhrases
{
    ...
    {
        return db_get_row('SELECT * FROM ?:csc_live_search_phrases WHERE phrase_id=?i', $phrase_id);
    }

    public static function _get_phrases_for_search($params, $ls_settings, $lang_code)
    {
        $condition = db_quote('phrase LIKE ?l AND phrase NOT LIKE ?l AND lang_code=?s', $params['q'] . '%', $params['q'], $lang_code);
        if (!empty($params['runtime_company_id'])) {
            $condition .= db_quote(' AND company_id=?i', $params['runtime_company_id']);
        }
        return db_get_array("SELECT phrase as q FROM ?:csc_live_search_phrases WHERE {$condition} LIMIT 10");
    }

    public static function _get_featured_products($params, $ls_settings, $lang_code)
    {
        $condition = db_quote(' phrase LIKE ?l AND lang_code=?s', $params['q'], $lang_code);
        if (!empty($params['runtime_company_id'])) {
            $condition .= db_quote(' AND company_id=?i', $params['runtime_company_id']);
        }
        $product_ids = db_get_field("SELECT product_ids FROM ?:csc_live_search_phrases WHERE {$condition}");
        if ($product_ids) {
