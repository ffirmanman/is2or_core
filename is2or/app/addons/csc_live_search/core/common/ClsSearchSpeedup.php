...
class ClsSearchSpeedup
{
    public static function _get_rest_of_products($step = 500)
    {
        ...
        // default cs-cart hook type
        fn_set_hook('cls_speedup_get_products', $join, $condition, $step);
        $products = db_get_array("SELECT ?:products.product_id, ?:products.product_code, ?:products.company_id, ?:product_descriptions.*
			FROM ?:products 
        ...
        $found_rows = db_get_field("SELECT COUNT(*)	FROM ?:products 
			...
			{$join}
			WHERE 1 {$condition}");
        return [$products, $found_rows];
    }

    public static function _delete_updated_products_indexes($timestamp = 0)
    {
        static $done;
        if (!$done) {
            for ($i = 0; $i < 10; $i++) {
                db_query("DELETE ?:csc_search_speedup_products_clusters_{$i} FROM ?:csc_search_speedup_products_clusters_{$i} LEFT JOIN ?:products ON ?:products.product_id = ?:csc_search_speedup_products_clusters_{$i}.product_id WHERE ?:products.updated_timestamp > ?i", $timestamp);
            }
            $done = true;
        }
    }
    ...
    public static function _speedup_scan_products($partial = false)
    {
        ...
        $clusters = db_get_hash_array('SELECT cluster_id, cluster FROM ?:csc_search_speedup_clusters', 'cluster');
        if ($partial) {
            $step = 500;
        } else {
            self::_delete_updated_products_indexes(fn_get_storage_data('csc_speedup_last_scan'));
            $step = 500;
        }
        ...
        if (!$products) {
            fn_set_storage_data('csc_speedup_last_scan', TIME);
            fn_set_notification('N', __('notice'), __('css.scaner_finish'));
            return false;
        }
        self::_save_indexation($products);
    ...
    public static function _save_indexation($products)
    {
        $data = [];
        $descriptions = [];
        $addon = CscLiveSearch::_get_option_values(true);
        foreach ($products as $product) {
            ...
            if (fn_allowed_for('ULTIMATE')) {
                $addon = CscLiveSearch::_get_option_values(true, $product['company_id']);
            }
            ...
            if (!empty($addon['search_products_on_ab__custom_h1']) && $addon['search_products_on_ab__custom_h1'] == 'Y' && !empty($product['ab__custom_product_h1'])) {
                $search_string .= ' ' . $product['ab__custom_product_h1'];
            }

            self::_prepare_additional_by_product($addon, $product['product_id'], $search_string, $product['lang_code']);
            ...
            $search_string = ClsSearchProducts::_prepare_query($search_string, $addon);
    ...
    public static function _prepare_additional_by_product($addon, $product_id, &$search_string, $lang_code)
    {
        ...
        if ($addon['search_on_features'] == 'Y') {
            ...
            if (!empty($addon['search_products_on_ab__custom_h1']) && $addon['search_products_on_ab__custom_h1'] == 'Y' && Registry::get('addons.ab__custom_h1.status') == 'A') {
                $fields .= ', ab__custom_feature_variant_h1';
            }
            $features = db_get_array("SELECT {$fields} FROM ?:product_features_values LEFT JOIN ?:product_feature_variant_descriptions ON ?:product_feature_variant_descriptions.variant_id=?:product_features_values.variant_id WHERE product_id=?i {$cond}", $product_id);
            $vals = [];
            foreach ($features as $feature) {
                ...
                if (!empty($feature['ab__custom_feature_variant_h1'])) {
                    $vals[$feature['ab__custom_feature_variant_h1']] = $feature['ab__custom_feature_variant_h1'];
                }
            }
            if ($vals) {
        ...
        if ($addon['search_on_pcode'] == 'Y' && version_compare(PRODUCT_VERSION, '4.12.1', '<')) {
            $option_product_codes = db_get_fields('SELECT product_code FROM ?:product_options_inventory WHERE product_id=?i', $product_id);
            if ($option_product_codes) {
                $search_string .= ' ' . implode(' ', $option_product_codes);
            }
        }
        fn_cls_hook_function('hooks_get_speedup_product_data', $addon, $product_id, $search_string, $lang_code);
    }

    public static function _prepare_product_clusters($addon, $product_id, $search_string, $data = [], $clusters = [])
    {
        ...
        foreach ($words as $word) {
            ...
            if (mb_strlen($word, 'utf-8') >= $addon['speedup_cluster_size']) {
                ...
                $data[self::_group_id($cluster_id)][$product_id . '-' . $cluster_id] = [
    ...
    {
        $del = [' ', ';', '-', '/', ',', '>', '_'];
        return explode($del[0], str_replace($del, $del[0], $string));
    }
    ...
    {
        $q = ClsSearchProducts::_prepare_query($q, $ls_settings);
