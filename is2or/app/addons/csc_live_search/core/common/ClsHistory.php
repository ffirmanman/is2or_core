...
class ClsHistory
{
    ...
    public static function _get_per_word($params, $items_per_page = 20)
    {
        ...
        $join = db_quote(' LEFT JOIN ?:csc_live_search_q_base ON ?:csc_live_search_q_base.qid=?:csc_live_search_q_requests.qid 
        ...
        $params['total_items'] = db_get_field("SELECT COUNT(DISTINCT(?:csc_live_search_q_base.qid)) FROM ?:csc_live_search_q_requests {$join} WHERE 1 {$condition}");
        ...
        $history = db_get_array("SELECT ?:csc_live_search_q_base.*, COUNT(?:csc_live_search_q_products.pid) as count_products,
			...
			FROM ?:csc_live_search_q_requests
			{$join}
