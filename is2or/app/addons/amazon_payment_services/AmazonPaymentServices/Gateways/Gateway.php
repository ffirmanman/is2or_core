...
class Gateway{
	...
	public function formatMinorAmount($amount){
	...
	public function getReturnUrl(){
		...
		if( !empty($this->order_id) )
			return fn_url("payment_notification.aps_return?merchant_reference={$this->reference}", AREA, 'current');
	...
	public function processResponse($post = null){
		...
		if( empty($response_code) )
			...
			if( isset($post['signature']) && trim($post['signature']) != ($post['generated_response_signature'] ?? $this->generateSignature($post,true)) ){
		...
		if ( $resType == 'CHECK_STATUS_SUCCESS' && !empty($post['order_status'])) {
		...
		if ( in_array($resType,['CAPTURE_SUCCESS','REFUND_SUCCESS']) ){
			...
			$captured_amount = !empty($details['captured_amount']) ? floatval($details['captured_amount']) : 0;
			$refunded_amount = !empty($details['refunded_amount']) ? floatval($details['refunded_amount']) : 0;

			$done_amount = number_format(floatval($post['amount'])/100,'2','.','');
			
			if( $resType == 'CAPTURE_SUCCESS' && $captured_amount ){
			...
			if( $resType == 'REFUND_SUCCESS' && $refunded_amount ){
	...
	public function generateReference($order_id = null){
		...
		$reference = $order_id.strtoupper(substr(hash("sha256",uniqid($hex.time(), true)),0,8));
	...
	public function processRequest(){
        ...
        $res['details'] = [
			...
			'amount' => $this->amount,
			'captured_amount' => $command == 'PURCHASE' ? $this->amount : 0,
        ];
	...
	public function checkUpdateOrder($gateway,$reference,$details){
		...
		if( !empty($resp['transaction_code']) ){
			...
			$details['response_type'] = $resType;
			
			$command = in_array($resType,['CAPTURE_SUCCESS','PAYMENT_SUCCESS']) ? 'CAPTURE' : 'AUTHORIZATION';
			...
			if( !empty($resp['authorized_amount']) || !empty($resp['captured_amount']) )
				$details['amount'] = $this->formatMinorAmount(!empty($resp['authorized_amount']) ? $resp['authorized_amount'] : $resp['captured_amount']);

			if( !empty($resp['captured_amount']) )
				$details['captured_amount'] = $this->formatMinorAmount($resp['captured_amount']);

			if( !empty($resp['refunded_amount']) )
				$details['refunded_amount'] = $this->formatMinorAmount($resp['refunded_amount']);
