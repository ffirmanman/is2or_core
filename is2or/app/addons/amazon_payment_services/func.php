   ...
   if (!empty($order_id) && !empty($gateway)) {
      ...
      if (fn_check_payment_script('aps.php', $order_id) && !empty($order_info['payment_method']['processor_params'])) {
         ...
         if ($alreadyPaid && $mode == 'return') {
            ...
            $aps = new \AmazonPaymentServices\Gateways\Gateway('', $order_info['payment_method']['processor_params']);

            $response = $aps->checkUpdateOrder($order_info['aps_gateway'], $order_info['aps_reference'], $order_info['payment_info']);

            if (
               !empty($response['order_status'])
               && $response['order_status'] == 'P'
               && !empty($response['response_type'])
               && $response['response_type'] == 'AUTHORIZATION_SUCCESS'
            ) {
               // capture payment
               list(,,$resp) = fn_amazon_payment_services_order_actions($order_id, 'capture', $order_info['payment_info']['amount']);
               $response['command'] = 'CAPTURE';
               $response['generated_response_signature'] = $aps->generateSignature($resp, true);

               $response['captured_amount'] = $order_info['payment_info']['captured_amount'];

               $response = array_merge($response, $resp);
            }

            // set response parameter
            $aps->setResponse($response);
            // handle return response from payfort
            $res = $aps->processResponse();
   ...
   if (fn_check_payment_script('aps.php', $order_id) && in_array($action, ['void', 'refund', 'capture'])) {
      ...
      if (!$error) {
         ...
         if ($action != 'void') {
            ...
            $params['currency'] = !empty($payment_info['currency']) ? trim($payment_info['currency']) : $order_info['secondary_currency'];
         }
         ...
         if (!empty($resp['error']))
            ...
            if (isset($resp['response_message']) && $resp['response_message'] == 'Success') {
               ...
               if ($action == 'void' || ($action == 'refund' && $payment_info['refunded_amount'] == $payment_info['captured_amount']))
   ...
   $list = [
      ...
      'Tabby' => [
         'tabby_enabled' => [
            'label' => __('aps_enabled'),
            'type' => 'checkbox',
            'hint' => __('aps_tabby_supported_currencies_hint'),
         ],
      ],
