...
class RowColumnInformation
{
    ...
    public static function COLUMN($cellAddress = null, ?Cell $cell = null)
    {
        ...
        if ($cell != null) {
            list(,, $sheetName) = Helpers::extractWorksheet($cellAddress, $cell);
            list(,, $cellAddress) = Helpers::extractCellAddresses($cellAddress, true, $cell->getWorksheet(), $sheetName);
        }
        list(, $cellAddress) = Worksheet::extractSheetTitle($cellAddress, true);
