...
class ReferenceHelper
{
    ...
    public function insertNewBefore(
        ...
        if ($this->cellReferenceHelper === null
        ...
        list($beforeColumn, $beforeRow) = Coordinate::indexesFromString($beforeCellAddress);
        ...
        $missingCoordinates = array_filter(
            array_map(
                function ($row) use ($highestColumn) {
            ...
            if (!empty($tableRange)) {
                if ($numberOfColumns !== 0) {
                    ...
                    if (count($tableColumns) > 0) {
                        ...
                        sscanf($beforeCellAddress, '%[A-Z]%d', $column, $row);
                        $columnIndex = Coordinate::columnIndexFromString((string) $column);
                        list($rangeStart, $rangeEnd) = Coordinate::rangeBoundaries($tableRange);
                        if ($columnIndex <= $rangeEnd[0]) {
                            if ($numberOfColumns < 0) {
