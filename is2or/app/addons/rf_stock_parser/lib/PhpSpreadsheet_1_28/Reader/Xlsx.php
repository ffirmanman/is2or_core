...
class Xlsx extends BaseReader
{
    ...
    public function canRead(string $filename): bool
    {
        ...
        if ($zip->open($filename) === true) {
            list($workbookBasename) = $this->getWorkbookBaseName();
    ...
    protected function loadSpreadsheetFromFile(string $filename): Spreadsheet
    {
        ...
        list($workbookBasename, $xmlNamespaceBase) = $this->getWorkbookBaseName();
                                        ...
                                        switch ((string) $definedName['name']) {
                                            ...
                                            case '_xlnm.Print_Area':
                                                ...
                                                foreach ($rangeSets as $rangeSet) {
                                                    list(, $rangeSet) = Worksheet::extractSheetTitle($rangeSet, true);
                                                    if (empty($rangeSet)) {
                                                        continue;
                                                    }
                        ...
                        if ($xmlWorkbook->definedNames) {
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                ...
                                if ((string) $definedName['localSheetId'] != '') {
                                    ...
                                    // Extract sheet name
                                        list($extractedSheetName) = Worksheet::extractSheetTitle((string) $definedNameValueParts[0], true); // @phpstan-ignore-line
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->b)) {
                            ...
                            if ((isset($attr['val']) && self::boolean((string) $attr['val']))
                                || (!isset($attr['val']))
                            ) {
