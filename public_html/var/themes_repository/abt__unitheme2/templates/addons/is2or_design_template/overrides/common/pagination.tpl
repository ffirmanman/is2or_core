...
{if $pagination.total_pages > 1}
    ...
    {if !$config.tweaks.disable_dhtml || $force_ajax}
        {assign var="ajax_class" value="cm-ajax cm-ajax-force cm-ajax-full-render"}
    {/if}
    ...
    <div class="ty-pagination">
        ...
        {/if}
        <a data-ca-scroll=".cm-pagination-container" class="{if $smarty.const.CART_LANGUAGE == 'ar'}ty-pagination__right-arrow {/if} ty-pagination__item ty-pagination__btn {if $pagination.prev_page}ty-pagination__prev cm-history {$ajax_class}{/if}" {if $pagination.prev_page}href="{"`$c_url`&page=`$pagination.prev_page`"|fn_url}" data-ca-page="{$pagination.prev_page}" data-ca-target-id="{$id}"{/if}><i class="ty-pagination__text-arrow"></i>&nbsp;<span class="ty-pagination__text">{__("prev_page")}</span></a>
