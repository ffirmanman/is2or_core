{if
    !$hidden_homepage && !$hide_vendor_store && !(strpos($smarty.request.dispatch, 'companies.home') === 0) &&
    fn_abt__ut2_mv_is_vendor_store_available($vendor_info.company_id) &&
    fn__abt__ut2_mv_get_company_microstore_homepage_blocks(null, ['status' => 'ObjectStatuses::ACTIVE'])
}
    <div class="ut2__buyer-motivation--item">
        ...
        <div class="ut2__buyer-motivation--items-wrap">
            ...
            <span><a href="{"companies.home?company_id=`$vendor_info.company_id`"|fn_url}">{__("abt__ut2_mv.view_vendor_home_page")}</a></span>
        </div>
    </div>
{/if}
    ...
    <div class="ut2__buyer-motivation--items-wrap">
        ...
        <span>
            {foreach $vendor_info.duration as $period_type => $duration}
                {if $duration}{__("abt__ut2_mv.n_`$period_type`",[$duration])} {/if}
            {/foreach}
        </span>
    </div>
</div>
