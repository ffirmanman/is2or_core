            ...
            {hook name="checkout:minicart"}
                <div class="cm-cart-content {if $block.properties.products_links_type == "thumb"}cm-cart-content-thumb{/if} {if $block.properties.display_delete_icons == "YesNo::YES"|enum}cm-cart-content-delete{/if}" id="cart_content_{$dropdown_id}">
                                                            ...
                                                            {if $block.properties.display_delete_icons == "YesNo::YES"|enum}
                                                                {if (!$runtime.checkout || $force_items_deletion) && !$product.extra.exclude_from_calculate}
                                                                    <div class="ut2-minicart__rm-btn">
                                                                        {*{include file="buttons/button.tpl" but_href="checkout.delete.from_status?vendor_id=`$product.company_id`&cart_id=`$key`&redirect_url=`$r_url`" but_meta="cm-ajax cm-ajax-full-render ut2-cart-item-delete" but_target_id="cart_status*" but_role="delete" but_name="delete_cart_item"}*}
                    ...
                    {if $block.properties.display_bottom_buttons == "YesNo::YES"|enum}
                        <div class="cm-cart-buttons buttons-container {if $cart.amount} full-cart{else} hidden{/if}">
                            {hook name="checkout:cart_subtotal"}
                            {if $cart.amount > 0 || $product.extra.buy_together}
                                <div class="cart_subtotal">
                                    <div class="ut2-cart_subtotal__caption">{__("total_items")}:</div>
                                    <div class="ut2-cart_subtotal__data">{$cart.amount}&nbsp;{__("items")} {__("for")}&nbsp;&nbsp;<strong>{include file="common/price.tpl" value=$cart.display_subtotal}</strong></div></div>
                            {/if}
                            {/hook}
