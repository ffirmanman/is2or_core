...
<div id="scroll_list_{$block.block_id}" class="ab-scroller-pd grid-list active-scroll owl-carousel ty-scroller-list ty-scroller"
     style="
            --gl-lines-in-name-product: {$settings.abt__ut2.product_list.$tmpl.lines_number_in_name_product[$settings.abt__device]};
            ...
            {if $tbh}--gl-thumbs-height: {$tbh};
            {/if} {if $tbw}--gl-thumbs-width: {$tbw};{/if}
     ">
    {foreach from=$items item="product" name="for_products"}
        {hook name="products:product_scroller_advanced_list"}
            <div class="ut2-gl__item{if $settings.abt__ut2.product_list.$tmpl.show_content_on_hover[$settings.ab__device] === "YesNo::YES"|enum} content-on-hover{/if}{if $settings.abt__ut2.product_list.decolorate_out_of_stock_products == "YesNo::YES"|enum && $product.amount <= 0} out-of-stock{/if}">
...
<script>
    (function(_, $) {
        $.ceEvent('on', 'ce.commoninit', function(context) {
            ...
            {if $block.properties.item_quantity > 5}
                {$item_quantity = 5}
            {/if}

            var item = {$item_quantity},
                itemsDesktop = {$item_quantity|default:5},
                itemsDesktopSmall = 4,
                itemsTablet = 3,
