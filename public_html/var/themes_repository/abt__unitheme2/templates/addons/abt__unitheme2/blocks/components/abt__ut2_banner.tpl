...
{$device = ""}
{if !empty($settings.ab__device) && $settings.ab__device != "desktop"}
    {$device = "_`$settings.ab__device`"}
{/if}
...
<div class="ut2-banner ut2-banner-{$b.abt__ut2_device_settings} {if $block.properties.navigation == "L"} ut2-scroll-item {/if}{$b.abt__ut2_class}" style="{strip}
    ...
    {if strlen(trim($b.abt__ut2_title))}
        ...
        --ab-banner-title-color: {if $b["abt__ut2`$device`_title_color_use"] === "YesNo::YES"|enum}{$b.abt__ut2_title_color|default:black}{elseif $b.abt__ut2_color_scheme === "dark"}white{else}black{/if};
    {/if}
