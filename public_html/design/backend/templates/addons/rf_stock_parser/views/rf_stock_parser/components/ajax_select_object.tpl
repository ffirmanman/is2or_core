...
{capture name="ajax_select_content"}
    ...
    <a {if $span_wrapping == false}id="sw_{$id}_wrap_"{/if} class="{if $type != "list"}btn btn-link link--monochrome{/if} dropdown-toggle" data-toggle="dropdown">
        {if $span_wrapping}
            <span id="sw_{$id}_wrap_">{$text|truncate:140:"...":true}</span>
            {if $dropdown_icon}
            ...
            <div class="ajax-popup-tools" id="scroller_{$id}">
                <ul class="cm-select-list" id="{$id}">
                    {foreach from=$objects key="object_id" item="item"}
                        ...
                        {else}
                            {assign var="name" value=$item.name|truncate:140:"...":true}
                        {/if}
                        <li>
