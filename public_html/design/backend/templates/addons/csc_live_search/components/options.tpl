        ...
        {foreach from=$tab item="field" key="field_name"}
           {if $field.type == 'input'}
            ...
            {elseif $field.type == 'checkbox'}
                <div class="control-group {if $field.group} grouped hidden {$field.group}{/if} {if $field.show_when}{$prefix}ShowWhen{/if}" id="container_elm_{$field_name}">
                    <label for="elm_{$field_name}" class="control-label{if $field.required} cm-required{/if} {if $field.label_class}{$field.label_class}{/if}">{if $field.title}{$field.title nofilter}{else}{__("`$prefix`.{$field_name}")}{/if}
                        {if $field.tooltip}{include file="common/tooltip.tpl" tooltip=__("`$prefix`.`$field_name`_tooltip")}{/if}:
                    </label>
                    <div class="controls">
            ...
            {elseif $field.type == 'title'}
                <h4 class="subheader {if $field.group} grouped hidden {$field.group}{/if} {if $field.show_when}{$prefix}ShowWhen{/if}" id="container_elm_{$field_name}">{if $field.title}{$field.title nofilter}{else}{__("`$prefix`.title_`$field_name`")}{/if}{if $field.tooltip}{include file="common/tooltip.tpl" tooltip=__("`$prefix`.`$field_name`_tooltip")}{/if}</h4>
            ...
            {elseif $field.type == 'func_info'}      	
             ...
             {elseif $field.type == 'link'}      	
                  <div class="control-group {if $field.group} grouped hidden {$field.group}{/if} {if $field.show_when}{$prefix}ShowWhen{/if}" id="container_elm_{$field_name}">
                      <label for="elm_{$field_name}" class="control-label">{if $field.label}{__("`$prefix`.{$field_name}")}
                        {if $field.tooltip}{include file="common/tooltip.tpl" tooltip=__("`$prefix`.`$field_name`_tooltip")}{/if}:{/if}</label>
                      <div class="controls">
