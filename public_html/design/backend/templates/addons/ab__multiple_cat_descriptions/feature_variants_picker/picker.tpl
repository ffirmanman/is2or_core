{assign var="data_id" value=$data_id|default:"feature_variants_list"}
{if !$rnd}{$rnd=rand()}{/if}
...
{if $show_add_button}
{if $multiple}
{assign var="_but_text" value=$but_text|default:__("ab__mcd.add_feature_variants")}
...
{/if}
<div class="pull-left">
{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="product_features.picker?display=`$display`&picker_for=`$picker_for`&extra=`$extra_var`&checkbox_name=`$checkbox_name`&root=`$default_name`&except_id=`$except_id`&data_id=`$data_id``$extra_url`"|fn_url but_text=$_but_text but_role=$_but_role but_target_id="content_`$data_id`" but_meta="cm-dialog-opener btn" but_icon="icon-plus"}
</div>
<div class="pull-right">
<a onclick="Tygh.$.cePicker('delete_js_item', '{$data_id}', 'delete_all', 'm'); return false;">{__("ab__mcd.remove_all")}</a>
</div>
{/if}
...
{/if}
<div class="hidden" id="content_{$data_id}" title="{$but_text|default:__("ab__mcd.add_feature_variants")}">
</div>
...
<th width="100%">{__("ab__mcd.feature_variant")}</th>
<th>&nbsp;</th>
</tr>
</thead>
...
{include file="addons/ab__multiple_cat_descriptions/feature_variants_picker/js.tpl" variant_id=$c_id holder=$data_id input_name=$input_name clone=false hide_link=$hide_link first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration+$start_pos}
{foreachelse}
{include file="addons/ab__multiple_cat_descriptions/feature_variants_picker/js.tpl" variant_id="" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link}
{/foreach}
{/if}
{if $multiple}
{include file="addons/ab__multiple_cat_descriptions/feature_variants_picker/js.tpl" variant_id="`$ldelim`variant_id`$rdelim`" holder=$data_id input_name=$input_name radio_input_name=$radio_input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
{/if}
{if $multiple}
</tbody>
