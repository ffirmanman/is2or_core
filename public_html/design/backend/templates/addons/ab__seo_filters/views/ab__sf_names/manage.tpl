...
<thead>
<tr>
<th width="1%" class="left mobile-hidden">
...
<th width="20%"><nobr><a class="cm-ajax" href="{"`$c_url`&sort_by=show_in_breadcrumbs&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("ab__sf.name.field.show_in_breadcrumbs")}{if $search.sort_by == "fixed"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a>{include file="common/tooltip.tpl" tooltip=__("ab__sf.name.field.show_in_breadcrumbs__tooltip")}</nobr></th>
<th width="5%"><a class="cm-ajax" href="{"`$c_url`&sort_by=is_noindex&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("ab__sf.name.field.is_noindex")}{if $search.sort_by == "is_noindex"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
<th width="5%">&nbsp;</th>
</tr>
</thead>
{foreach from=$ab__sf_names item="n"}
<tr>
<td class="left mobile-hidden" ><input type="checkbox" name="sf_id[]" value="{$n.sf_id}" class="checkbox cm-item cm-item-status-{$r.status|lower}" /></td>
<td data-th="{__("ab__sf.name.field.category")}">{$n.category_id|fn_get_category_name:$smarty.const.DESCR_SL}</td>
<td data-th="{__("ab__sf.name.field.name")}"><a target="_blank" href="{"categories.view&category_id=`$n.category_id`&features_hash=`$n.features_hash`"|fn_url:"C":"current":$smarty.const.DESCR_SL}">{$n.name}&nbsp;»</a></td>
<td data-th="{__("ab__sf.name.field.features_hash")}">
...
<td class="center" data-th="{__("ab__sf.name.field.fixed")}">
...
<td class="center" data-th="{__("ab__sf.name.field.show_in_breadcrumbs")}">
{$n.show_in_breadcrumbs}
</td>
<td data-th="{__("ab__sf.name.field.is_noindex")}">{$n.is_noindex}</td>
<td class="nowrap">
...
<li>{btn type="list" text=__("clone") class="cm-confirm cm-post" href="ab__sf_names.clone?sf_id=`$n.sf_id`"}</li>
<li class="divider"></li>
{if $n.fixed == "ABSFConfigs::PAGE_STATE_FIXED"|enum}
...
<li class="divider"></li>
<li>{btn type="list" text=__("ab__sf.name.do_noindexed") dispatch="dispatch[ab__sf_names.do_noindexed]" form="manage_ab__sf_names_form"}</li>
<li>{btn type="list" text=__("ab__sf.name.do_indexed") dispatch="dispatch[ab__sf_names.do_indexed]" form="manage_ab__sf_names_form"}</li>
<li class="divider"></li>
