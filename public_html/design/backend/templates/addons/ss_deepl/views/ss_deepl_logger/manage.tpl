...
{if $logger_data}
    ...
    <tr>
        ...
        <th width="15%" style="text-align:center"><a class="cm-ajax" href="{"`$c_url`&sort_by=date_add&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("`$addon_id`_logger_date")}{if $search.sort_by == date_add}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        <th width="35%" style="text-align:center"><a class="cm-ajax" href="{"`$c_url`&sort_by=message&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("`$addon_id`_logger_message")}{if $search.sort_by == message}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        <th width="10%" style="text-align:center"><a class="cm-ajax" href="{"`$c_url`&sort_by=call_function&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("`$addon_id`_logger_call_function")}{if $search.sort_by == call_function}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        <th width="10%" style="text-align:center"><a class="cm-ajax" href="{"`$c_url`&sort_by=dispatch&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("`$addon_id`_logger_dispatch")}{if $search.sort_by == dispatch}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        <th width="8%" style="text-align:center"><a class="cm-ajax" href="{"`$c_url`&sort_by=log_level&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("`$addon_id`_logger_action")}{if $search.sort_by == log_level}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
...
{/capture}
{include file="common/mainbox.tpl" title=__("`$addon_id`_logger_list") sidebar=$smarty.capture.sidebar  content=$smarty.capture.mainbox buttons=$smarty.capture.buttons}
