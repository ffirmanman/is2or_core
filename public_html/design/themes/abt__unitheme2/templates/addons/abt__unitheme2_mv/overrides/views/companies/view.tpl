...
{$obj_id_prefix="`$obj_prefix``$obj_id`"}
    ...
    <div class="ty-company-detail clearfix">
        ...
        {capture name="tabsbox"}
            ...
            {hook name="companies:tabs"}{/hook}
        {/capture}
    </div>

    {capture name="tabsbox_with_content"}
        {include file="common/tabsbox.tpl" content=$smarty.capture.tabsbox active_tab=$smarty.request.selected_section}
    {/capture}
    {capture name="tabsbox_with_content"}
        {'/id="(content_.*?)"/'|preg_replace:"id=\"\$1_`$obj_id_prefix`\"":$smarty.capture.tabsbox_with_content nofilter}
    {/capture}
    {capture name="tabsbox_with_content"}
        {'/<li id="(.*?)" class="ty-tabs__item cm-js/'|preg_replace:"<li id=\"\$1_`$obj_id_prefix`\" class=\"ty-tabs__item cm-js":$smarty.capture.tabsbox_with_content nofilter}
    {/capture}
        {$smarty.capture.tabsbox_with_content nofilter}
