...
{if $return_info.return_id}
    {assign var="refund_evidences" value=$return_info.return_id|fn_get_is2or_evidences}

    {if $refund_evidences}
        <div class="ty-rma-evidence-section" style="margin-top: 20px; border-top: 1px solid #e1e1e1; padding-top: 20px;">
            <h3 class="ty-subheader">{__("is2or_refund_evidence")}</h3>
            
            <div class="ty-rma-evidence-gallery">
                {foreach from=$refund_evidences item="file"}
                    <div class="ty-rma-evidence-item">
                        {if $file.file_type == "photo"}
                            <a class="cm-image-previewer" href="{"var/files/refund/`$file.return_id`/`$file.filename`"|fn_url}">
                                <img src="{"var/files/refund/`$file.return_id`/`$file.filename`"|fn_url}" alt="Evidence" style="width: 100%; height: 140px; object-fit: cover;" />
                            </a>
                        {else}
                            <div class="ty-rma-video-wrapper">
                                <video width="100%" height="140" controls style="object-fit: cover;">
                                    <source src="{"var/files/refund/`$file.return_id`/`$file.filename`"|fn_url}" type="video/mp4">
                                </video>
                            </div>
                        {/if}
                    </div>
