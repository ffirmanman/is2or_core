...
{if $field.field_name == "s_city" || $field.field_name == "b_city"}
    ...
    (function($) {
        ...
        function attachAutocomplete() {
            var $original = $('#' + ELEMENT_ID);
            if (!$original.length || $original.data(DATA_KEY)) {
                return;
            }

            try {
                var inputClass = $original.attr('class');
                var inputVal   = $original.val();

                /* Sync original to visible */

                /* Create visible input for user interaction */
                var $visible = $('<input type="text" autocomplete="off" />')
                ...
                /* Attach Legacy Autocomplete to visible input */
                var ac = new google.maps.places.Autocomplete($visible[0], {
                    types: ['(cities)']
                });
                ac.addListener('place_changed', function() {
                    var place = ac.getPlace();
                    if (place && place.name) {
                        $visible.val(place.name);
                        $original.val(place.name).trigger('change');
                    }
                });

                /* User types manually → sync to original */
                $visible.on('input', function() {
                    $original.val($(this).val());
                });

                /*
                 * Watch for changes on original input from SPL (via $.val() does not trigger 'change')
                 * Use MutationObserver + lightweight polling to detect updates
                 * performed by SPL via $('#elm_X').val(...)
                 */
                var lastVal = inputVal;
                setInterval(function() {
                    var currentVal = $original.val();
                    if (currentVal !== lastVal) {
                        lastVal = currentVal;
                        $visible.val(currentVal);
                    }
                }, 300);

                $original.data(DATA_KEY, true);
        ...
        /* Use window.Tygh if available, fallback to direct jQuery */
        function bindEvents() {
            ...
            /* Always attach via document.ready as fallback */
