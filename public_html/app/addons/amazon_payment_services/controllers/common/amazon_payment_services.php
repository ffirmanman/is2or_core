...
if( $mode == 'applepay_checkout'){
	...
	exit;
} if ($mode == 'redirect_url' && !empty($_REQUEST['order_id']) && !empty($_REQUEST['user_id'])) {
	$order_id = base64_decode($_REQUEST['order_id']);
	$user_id = base64_decode($_REQUEST['user_id']);
	
	$order_info = fn_get_order_info($order_id);

	$gateway = new \AmazonPaymentServices\Gateways\Gateway(
		'',
	...
	$submit_url = $gateway->getCheckoutUrl();
	...
	$params = [
		...
		'merchant_reference' => $gateway->generateReference($order_info['order_id']),
		'amount' => $gateway->getAmount(),
		'currency' => $currency,
		'language' => $gateway->getLanguage(),
		'customer_email' => fn_get_user_email($user_id),
		'customer_name' => fn_get_user_name($user_id),
		'order_description' => "Order #{$order_id}",
		'return_url' => fn_url('amazon_payment_services.callback?order_id=' . base64_encode($order_id), 'C'),
	];

	$params['merchant_extra1'] = $order_id;
	$params['merchant_extra2'] = $user_id;
	...
	db_query("UPDATE ?:orders SET ?u WHERE order_id = ?i",[
		'aps_reference' => trim($params['merchant_reference']),
		'aps_gateway'   => 'cc', //Static for now
	], $order_id);

	fn_update_order_payment_info($order_id, [
		'gateway'=> 'cc',
		'payment_mode'=> $params['command'],
		'merchant_reference' => isset($params['merchant_reference']) ? trim($params['merchant_reference']) : '',
		'amount' => $order_info['total'],
		'captured_amount' => $params['command'] == 'PURCHASE' ? $order_info['total'] : 0,
	]);
...
EOT;
	...
	echo(__('text_cc_processor_connection', array(
		'[processor]' => "Amazon Payment Services",
	)));
...
<html lang="en-US">
    ...
    <body>
        ...
        <script>window.location = "index.php?dispatch=payment_notification.success&payment=aps&order_id='. $order_id .'";</script>
    </body>
</html>');
    die;
}
