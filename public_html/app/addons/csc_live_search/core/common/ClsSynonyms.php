...
class ClsSynonyms
{
    ...
    public static function _update_synonym($synonym_data, $synonym_id = 0)
    {
        ...
        $synonym_data['synonyms'] = json_encode($synonym_data['synonyms'], JSON_UNESCAPED_UNICODE);
    ...
    public static function _get_search_synonyms($q, $ls_settings, $params)
    {
        ...
        if (!empty($ls_settings['use_synonyms'])) {
            if (!isset($synonyms[$q])) {
                $q = trim(db_quote('?s', $q), "'");
                $condition = 'status="A"';
                if (empty($ls_settings['ignore_lang_code']) || $ls_settings['ignore_lang_code'] != 'Y') {
                    $condition .= db_quote(' AND lang_code=?s', $params['lang_code']);
                }
                ...
                $synonyms[$q] = db_get_fields("SELECT phrase FROM ?:csc_live_search_synonyms WHERE {$condition}");
            }
            return $synonyms[$q];
        }
        return [];
