...
function db_query($query)
{
    ...
    $args = array_slice(func_get_args(), 1);
    list($result, $mysqli) = Connection::_dbq($query, ...$args);
    $id = $mysqli->insert_id;
...
function db_get_hash_array($query)
{
    ...
    $args = array_slice(func_get_args(), 1);
    $data = db_get_array($query, ...array_slice($args, 1));
    $_data = [];
    foreach ($data as $k => $row) {
        $_data[$row[$args[0]]] = $row;
    }
    return $_data;
}
...
function db_get_hash_single_array($query, $params)
{
    ...
    $_result = db_get_array($query, ...$args);
    $result = [];
...
function db_quote($query)
{
    ...
    return Connection::db_process($pattern, $args);
}
...
function db_connect()
{
    ...
    if (!$mysqli) {
        ...
        $mysqli = new mysqli($hosts[0], $config['db_user'], $config['db_password'], $config['db_name'], !empty($hosts[1]) ? $hosts[1] : null);
        // Kill previouse connection
        // $mysqli->kill(base64_decode($thread_id);
        ...
        $mysqli->query("SET NAMES 'utf8', sql_mode = '', SESSION group_concat_max_len = 3000");
...
class Connection
{
    ...
    public static function db_process($pattern, $data = [])
    {
        $config = fn_get_config_data();
        ...
        if (!empty($data) && preg_match_all('/\\?(i|s|l|d|a|n|u|e|m|p|w|f)+/', $pattern, $m)) {
            ...
            foreach ($m[0] as $k => $ph) {
                ...
                switch ($ph) {
                    // integer
                    case '?i':
                        $pattern = self::strReplace($ph, (int) $data[$k], $pattern, $_offset);
                    ...
                    case '?n':
                        $data[$k] = is_array($data[$k]) ? $data[$k] : [$data[$k]];
                        $pattern = self::strReplace($ph, !empty($data[$k]) ? implode(', ', array_map(['self', 'intVal'], $data[$k])) : "''", $pattern, $_offset);
    ...
    public static function getTableFields($table_name, $exclude = [], $wrap = false)
    {
    ...
    public static function _dbq($query)
    {

        $args = array_slice(func_get_args(), 1);
        $query = self::db_process($query, $args);
        $mysqli = db_connect();
        $result = $mysqli->query($query);
        if (!$result) {
            if (defined('CLS_DEBUG')) {
