...
. ' WHERE c.level = 1 AND c.status = ?s AND c.is_trash = ?s AND cd.category = ?s ?p', 'A', 'N', 'Electronics', fn_get_company_condition('c.company_id'));
if (empty($category_id)) {
$category_id = db_get_field('SELECT category_id FROM ?:categories WHERE level=1 AND status = ?s AND is_trash = ?s ?p', 'A', 'N', fn_get_company_condition());
}
if (empty($category_id)) {
