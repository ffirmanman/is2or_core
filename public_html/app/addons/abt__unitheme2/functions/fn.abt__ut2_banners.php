...
if (!empty($params['preview']) && $params['status'] == 'A'){
unset($params['status']);
$condition = str_replace(' AND ?:banners.status = \'A\'', '', $condition);
}
if (AREA == SiteArea::STOREFRONT && empty($params['preview'])) {
...
{
if ($_REQUEST['dispatch'] == 'banners.preview'){
$condition = str_replace(" AND ?:banners.status IN ('A', 'H') ", '', $condition);
}
