...
class Indirect
{
    ...
    {
        list($baseCol, $baseRow) = Coordinate::indexesFromString($cell->getCoordinate());
        ...
        list($cellAddress, $worksheet, $sheetName) = Helpers::extractWorksheet($cellAddress, $cell);
        ...
        try {
            list($cellAddress1, $cellAddress2, $cellAddress) = Helpers::extractCellAddresses($cellAddress, $a1, $cell->getWorkSheet(), $sheetName, $baseRow, $baseCol);
        } catch (\Exception $e) {
            return ExcelError::REF();
        }

        if ((!preg_match('/^' . Calculation::CALCULATION_REGEXP_CELLREF . '$/miu', $cellAddress1, $matches))
