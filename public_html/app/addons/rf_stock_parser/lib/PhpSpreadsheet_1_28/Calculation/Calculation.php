...
class Calculation
{
            ...
            if ($locale !== 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (strpos($localeFunction, '=') !== false) {
                        list($fName, $lfName) = array_map('trim', explode('=', $localeFunction));
                ...
                foreach ($localeSettings as $localeSetting) {
                    ...
                    if (strpos($localeSetting, '=') !== false) {
                        list($settingName, $settingValue) = array_map('trim', explode('=', $localeSetting));
    ...
    private function processTokenStack($tokens, $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIf'])) {
                ...
                if ((isset($storeValue) || $tokenData['reference'] === 'NULL')
            ...
            if ($token instanceof Operands\StructuredReference) {
                ...
                switch ($token) {
                    ...
                    //        matrix operation
                        ...
                        //    Ensure that both operands are arrays/matrices
                            list($rows, $columns) = self::checkMatrixOperands($operand1, $operand2, 2);
                ...
                if (is_array($arg)) {
                    $operand2 = $multiplier;
                    $result = $arg;
                    list($rows, $columns) = self::checkMatrixOperands($result, $operand2, 0);
                    for ($row = 0; $row < $rows; ++$row) {
                        for ($column = 0; $column < $columns; ++$column) {
    ...
    private function executeNumericBinaryOperation($operand1, $operand2, $operation, &$stack)
    {
        ...
        if (($this->validateBinaryOperand($operand1, $stack) === false)
    ...
    public function extractCellRange(&$range = 'A1', ?Worksheet $worksheet = null, $resetLog = true)
    {
        ...
        if ($worksheet !== null) {
            ...
            if (strpos($range, '!') !== false) {
                list($worksheetName, $range) = Worksheet::extractSheetTitle($range, true);
        ...
        if (count($methodArguments) > 0) {
            ...
            foreach ($emptyArguments as $argumentId => $isArgumentEmpty) {
                if ($isArgumentEmpty === true) {
                    ...
                    if (!array_key_exists($reflectedArgumentId, $methodArguments)
        ...
        if ($methodArgument->isDefaultValueAvailable()) {
            ...
            if ($methodArgument->isDefaultValueConstant()) {
                ...
                if (strpos($constantName, '::') !== false) {
                    list($className, $constantName) = explode('::', $constantName);
