...
class Engineering
{
     ...
     *
     * @param float $x   The value at which to evaluate the function.
     *                   If x is nonnumeric, BESSELY returns the
     *                   #VALUE! error value.
     * @param int   $ord The order of the Bessel function. If n is not an integer, it is truncated.
     *                   If $ord is nonnumeric, BESSELY returns the #VALUE! error value. If $ord <
     *                   0, BESSELY returns the #NUM! error value.
     *
     * @return array|float|string Result, or a string containing an error
     */
     ...
     *                      Number cannot contain more than 10 characters. The most
     *                      significant bit of number is the sign bit (40th bit from the
     *                      right). The remaining 9 bits are magnitude bits. Negative
     *                      numbers are represented using two's-complement notation. If
     *                      number is negative, HEX2BIN ignores places and returns a
     *                      10-character binary number. If number is negative, it cannot
     *                      be less than FFFFFFFE00, and if number is positive, it cannot
     *                      be greater than 1FF. If number is not a valid hexadecimal
     *                      number, HEX2BIN returns the #NUM! error value. If HEX2BIN
     *                      requires more than places characters, it returns the #NUM!
     *                      error value.
     ...
     *                      sign bit. The remaining 39 bits are magnitude bits. Negative numbers are
     *                      represented using two's-complement notation. If number is negative, HEX2OCT
     *                      ignores places and returns a 10-character octal number. If number is
     *                      negative, it cannot be less than FFE0000000, and if number is positive, it
     *                      cannot be greater than 1FFFFFFF. If number is not a valid hexadecimal
     *                      number, HEX2OCT returns the #NUM! error value. If HEX2OCT requires more than
     *                      places characters, it returns the #NUM! error value.
