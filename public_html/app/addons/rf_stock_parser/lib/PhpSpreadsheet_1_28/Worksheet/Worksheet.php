...
class Worksheet implements IComparable
{
    ...
    private static function checkSheetCodeName($sheetCodeName)
    {
        ...
        if ((str_replace(self::$invalidCharacters, '', $sheetCodeName) !== $sheetCodeName)
...
*/ $this->chartCollection,
     ...
     *                                            be updated to reflect the new sheet name. This should be
     *                                            left as the default true, unless you are certain that no
     *                                            formula cells on any worksheet contain references to this
     *                                            worksheet
     ...
     *                                                  Note that PhpSpreadsheet does not
     *                                                  validate that the value and datatype are
     *                                                  consistent, in using this method, then
     *                                                  it is your responsibility as an end-user
     *                                                  developer to validate that the value and
     *                                                  the datatype match. If you do mismatch
     *                                                  value and datatpe, then the value you
     *                                                  enter may be changed to match the
     *                                                  datatype that you specify.
     *
     * @see DataType
     *
     * @return $this
    ...
    public function getCell($coordinate): Cell
    {
        ...
        if ($this->cellCollection->has($cellAddress)) {
             ...
             * @var Cell $cell
             */
            return $this->cellCollection->get($cellAddress);
        }

        /**
         * @var Worksheet $sheet
         */
        list($sheet, $finalCoordinate) = $this->getWorksheetAndCoordinate($cellAddress);
    ...
    private function getWorksheetAndCoordinate(string $coordinate): array
    {
        ...
        } elseif (!preg_match('/^' . Calculation::CALCULATION_REGEXP_CELLREF . '$/i', $coordinate)
    ...
    {
        list($column, $row, $columnString) = Coordinate::indexesFromString($coordinate);
    ...
    public function duplicateStyle(Style $style, $range)
    {
        ...
        list($rangeStart, $rangeEnd) = Coordinate::rangeBoundaries($range . ':' . $range);
     ...
     *                                                  or passing in an array of [$fromColumnIndex,
     *                                                  $fromRow, $toColumnIndex, $toRow] (e.g. [3, 5, 6,
     *                                                  8]), or an AddressRange.
     * @param string                         $behaviour How the merged cells should behave.
     *                                                  Possible values are:
     *                                                  MERGE_CELL_CONTENT_EMPTY - Empty
     *                                                  the content of the hidden cells
     *                                                  MERGE_CELL_CONTENT_HIDE - Keep the
     *                                                  content of the hidden cells
     *                                                  MERGE_CELL_CONTENT_MERGE - Move the
     *                                                  content of the hidden cells into
     *                                                  the first cell
     *
     * @return $this
     */
     ...
     *                                                                      or passing in an array of [$fromColumnIndex,
     *                                                                      $fromRow, $toColumnIndex, $toRow] (e.g. [3, 5, 6,
     *                                                                      8]), or a CellAddress or AddressRange object.
     ...
     *                                                        or as an array of [$columnIndex, $row] (e.g.
     *                                                        [3, 5]), or a CellAddress object. Passing a
     *                                                        null value for this argument will clear any
     *                                                        existing freeze pane for this worksheet.
...
*/ $num
    ...
    public function setSelectedCells($coordinate)
    {
        ...
        if (Coordinate::coordinateIsRange($coordinate)) {
            list($first) = Coordinate::splitRange($coordinate);
