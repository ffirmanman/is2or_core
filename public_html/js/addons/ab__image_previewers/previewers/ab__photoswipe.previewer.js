...
(function (_, $) {
    ...
    function fn_display(elm) {
        ...
        var activeIndexImg = elm.data('caImageOrder') ?? ($(elm).closest(".owl-item.active").length ? $(elm).closest(".owl-item.active").index() : 0);
        if (activeIndexImg === -1 || elm.siblings(':first').data('caImageOrder') === -1) {
          activeIndexImg += 1;
        }
